 ...
 * @param {string=} [plainTextOptions.namedExport='plainText'] - Specify the name of named exporting or you can pass undefined to specify to use the default exporting.
 * @param {boolean=} [plainTextOptions.dtsAutoGen=false] - generate dts file for plain text file automatically.
 * @param {boolean=} [plainTextOptions.distAutoClean=false] - cleanup the generated dts file after this plugin startup.
  ...
  return {
    ...
    transform (code, id) {
      if (typeof match === 'function' ? match.call(this, code, id) : testMatch(match, id)) {
        ...
        // transform code
        {
          const exportCode = plainTextOptions.namedExport
            // Named exporting
          ...
          // ~~ effect ~~
          magicString.overwrite(0, code.length, exportCode)
        }
        ...
        if (plainTextOptions.dtsAutoGen) {
          ...
          if (fs.existsSync(dtsFile) && !fs.readFileSync(dtsFile, { encoding: 'utf-8' }).startsWith(dtsBanner)) {
            ...
            fs.writeFileSync(id + '.d.ts', typeDeclaration)
          }
